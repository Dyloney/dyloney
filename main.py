...
def populate_tree_view(tv):
    ...
    for i, item in enumerate(foodlist):
        g = tv.add_node(TreeViewButton(text='%s' % item.categ, font_size = '50sp'))
        tv.add_node(TreeViewButton(text='%s' % item.name, font_size = '40sp'), g)

class POSFMApp(App):

    def build(self):
        tv = TreeView(root_options=dict(text='Tree One'), hide_root=True, indent_level=100, indent_start=200)
        tv.size_hint = 1, None
